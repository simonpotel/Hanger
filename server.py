...
def main():
    ...
    class ServerApp:
        def __init__(self, config_path, update_interval):
            ...
            logger.info(f"ServerApp initialized with config_path: {
                        config_path} and update_interval: {update_interval}")
            logger.debug(f"ServerApp __init__ called with config_path={
                         config_path}, update_interval={update_interval}")
